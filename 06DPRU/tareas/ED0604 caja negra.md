...
Se te ha pedido que implementes las pruebas para una clase denominada *SalesList (lista de ventas)*, cuyo atributo es ***List\<Venta\>*** . Cada venta tiene un ` un importe: double, una cantidad: int y un precio: double.`

La clase tiene los siguientes métodos:

```java
//1. 
addSale(int cantidad, double precio)
```
> Añade un nuevo elemento a la ***Lista***. El **importe** se calculará multiplicando la **cantidad** y el **precio**. Devolverá 0 si todo está bien y -1 si hay algún error. **No** podremos sumar ventas con **importe = 0** o **precio \< 0**, pero sí podemos añadir ventas con importes negativos (**pero no precios negativos**).
